...
/****** 调试日志 ******/
#ifdef DEBUG
#define YHHLOG(...) printf(" %s\n",[[NSString stringWithFormat:__VA_ARGS__]UTF8String]);
#define YHHLOG_CURRENT_METHOD NSLog(@"%@-%@", NSStringFromClass([self class]), NSStringFromSelector(_cmd))
#else
#define YHHLOG(...) ;
#define YHHLOG_CURRENT_METHOD ;
#endif

// 是否横竖屏
// 用户界面横屏了才会返回YES
#define YHHIS_LANDSCAPE UIInterfaceOrientationIsLandscape([[UIApplication sharedApplication] statusBarOrientation])
// 无论支不支持横屏，只要设备横屏了，就会返回YES
#define YHHIS_DEVICE_LANDSCAPE UIDeviceOrientationIsLandscape([[UIDevice currentDevice] orientation])

// 屏幕宽度，会根据横竖屏的变化而变化
#define YHHSCREEN_WIDTH ([[UIScreen mainScreen] bounds].size.width)

// 屏幕宽度，跟横竖屏无关
#define YHHDEVICE_WIDTH (YHHIS_LANDSCAPE ? [[UIScreen mainScreen] bounds].size.height : [[UIScreen mainScreen] bounds].size.width)

// 屏幕高度，会根据横竖屏的变化而变化
#define YHHSCREEN_HEIGHT ([[UIScreen mainScreen] bounds].size.height)

// 屏幕高度，跟横竖屏无关
#define YHHDEVICE_HEIGHT (YHHIS_LANDSCAPE ? [[UIScreen mainScreen] bounds].size.width : [[UIScreen mainScreen] bounds].size.height)
...
#define IPhoneXSafeAreaInsets [YHHUIHelper yhh_safeAreaInsetsForIPhoneX]

// 获取顶部导航栏占位高度，从而在布局 subviews 时可以当成 minY 参考
// 注意，以下两个宏已废弃，请尽量使用 UIViewController (YHHUI) yhhui_navigationBarMaxYInViewCoordinator 代替
#define NavigationContentTop (StatusBarHeight + NavigationBarHeight)
#define NavigationContentStaticTop NavigationContentTop

// 获取一个像素
#define PixelOne [YHHUIHelper pixelOne]
...
// 字体相关的宏，用于快速创建一个字体对象，更多创建宏可查看 UIFont+YHHUI.h
...
// UIColor 相关的宏，用于快速创建一个 UIColor 对象，更多创建的宏可查看 UIColor+YHHUI.h
...
#define YHHUIViewAnimationOptionsCurveOut (7<<16)
#define YHHUIViewAnimationOptionsCurveIn (8<<16)
