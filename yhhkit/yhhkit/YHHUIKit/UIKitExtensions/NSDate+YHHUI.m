...
#import "NSDate+YHHUI.h"

@implementation NSDate (YHHUI)
...
+ (NSString *)yhhui_chineseCalendarWithDate:(NSDate *)date{
    return [self yhhui_chineseCalendarWithDate:date options:YHHChineseCalendarOptionDay|YHHChineseCalendarOptionMonth|YHHChineseCalendarOptionShuXiang|YHHChineseCalendarOptionTianGanDiZhi];
}
...
+(NSString *)yhhui_chineseCalendarWithDate:(NSDate *)date options:(YHHChineseCalendarOptions)options{
    ...
    NSMutableString *szNongli = [NSMutableString string];
    ...
    NSString * tempTianGan = [NSString stringWithCString:cTianGan[((wCurYear - 4) % 60) % 10] encoding:NSUTF8StringEncoding];
    NSString * tempDiZhi = [NSString stringWithCString:cDiZhi[((wCurYear - 4) % 60) % 12]  encoding:NSUTF8StringEncoding];
    if(options&YHHChineseCalendarOptionTianGanDiZhi){
        [szNongli appendString:tempTianGan];
        [szNongli appendString:tempDiZhi];
    }
    if(options&YHHChineseCalendarOptionShuXiang){
        [szNongli appendString:szShuXiang];
    };
    if(options&YHHChineseCalendarOptionTianGanDiZhi || options&YHHChineseCalendarOptionShuXiang){
        [szNongli appendString:@"年"];
    }
    //生成农历月
    if(options&YHHChineseCalendarOptionMonth){
        if (wCurMonth < 1) {
        ...
        [szNongli appendString:szNongliMonth];
    }
    //生成农历日
    if(options&YHHChineseCalendarOptionDay){
        NSString * tempDayName = [NSString stringWithCString:cDayName[wCurDay] encoding:NSUTF8StringEncoding];
        [szNongli appendString:tempDayName];
    }
    return szNongli;
}

-(NSString *)yhhui_toChineseCalendar{
    return [NSDate yhhui_chineseCalendarWithDate:self];
}

-(NSString *)yhhui_toChineseCalendarWithOptions:(YHHChineseCalendarOptions)options{
    return [NSDate yhhui_chineseCalendarWithDate:self options:options];
}
@end
